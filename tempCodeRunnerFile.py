...
#background img
bgimg = pygame.image.load('Space Invaders game\\space bg.png')

#background Music
mixer.music.load('Space Invaders game\\bg music.mp3')
mixer.music.play(-1)

#create icon
icon = pygame.image.load('Space Invaders game\\spaceship (1).png')
pygame.display.set_icon(icon)

#create player image:
playerimg = pygame.image.load(
    "Space Invaders game\\arcade-game.png")
player_x = 420
player_y = 600
playerx_change = 0
...
for i in range(num_enemies):
    enemyimg.append(pygame.image.load('Space Invaders game\\alien (1).png'))
...
bulletimg = pygame.image.load('Space Invaders game\\bullets.png')
...
while run:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE:
                if bullet_state is "ready":
                    bulletsound= mixer.Sound('Space Invaders game\\shooting.wav')
                    bulletsound.play()
                    bulletX = player_x
    ...
    for i in  range(num_enemies):
        ...
        if collison1:
            ...
            explosion_sound=mixer.Sound('Space Invaders game\\explosion sound.wav')
